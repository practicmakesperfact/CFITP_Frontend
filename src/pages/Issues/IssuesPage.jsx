...
export default function IssuesPage() {
  ...
  console.log("DEBUG - Issues data:", issuesData);

  const issues = issuesData?.results || [];

  const filteredIssues = (() => {
    console.log("DEBUG - Filtering issues for role:", userRole);
    ...
    if (userRole === "client") {
      ...
      const clientIssues = issues.filter((issue) => {
        ...
        console.log(
          `Issue ${issue.id}: reporter_email=${issue.reporter_email}, created_by_email=${issue.created_by_email}, matches=${matches}`
        );
        return matches;
      });

      console.log("DEBUG - Client issues found:", clientIssues.length);
      return clientIssues;
    }
    ...
    if (userRole === "staff") {
      ...
      const staffIssues = issues.filter((issue) => {
        ...
        console.log(
          `Issue ${issue.id}: assignee_email=${issue.assignee_email}, matches=${matches}`
        );
        return matches;
      });

      console.log("DEBUG - Staff issues found:", staffIssues.length);
      return staffIssues;
    }

    // For manager and admin, show all issues
    console.log("DEBUG - Manager/Admin, showing all issues");
    return issues;
  })();

  console.log("DEBUG - Filtered issues:", filteredIssues);
